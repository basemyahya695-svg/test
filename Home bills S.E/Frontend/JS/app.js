...
const copy = {
  en: {
    ...
    calendar: "Calendar",
    emailReminders: "Email monthly unpaid bills",
  ...
  ar: {
    ...
    calendar: "التقويم",
    emailReminders: "إرسال الفواتير غير المدفوعة بالبريد",
...
function bindBillActions(bills, renderer = renderBillsPage) {
  ...
  document.querySelectorAll("[data-action='edit-bill']").forEach((button) => {
    button.addEventListener("click", () => openBillEditor(bills.find((bill) => String(bill.id) === button.dataset.id), renderer));
  });
...
function openBillEditor(bill, renderer = renderBillsPage) {
